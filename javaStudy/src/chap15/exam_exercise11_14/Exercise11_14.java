package chap15.exam_exercise11_14;
...
//			menu = s.nextInt();	// 마지막에 '\n' 이 남는다. (남은게 오류 발생시킴)
			if (menu < 1 || menu > 3) { // 잘못 입력 받은 경우
				System.out.println("메뉴를 잘못 선택하셨습니다. 다시 입력해주세요.");
				continue;
			} else { // 올바른 메뉴를 입력했으면 반복문을 종료
				break;
			}
		} while (true);
		return menu;
	...
	static void inputRecord() {
		...
		while (true) { // 여러 학생의 성적을 입력 받기
			System.out.print(">>");
			/*
			 * (2) 아래의 로직에 맞게 코드를 작성하시오. 1. Scanner를 이용해서 화면으로 부터 데이터를 입력받는다. (','를구분자로 ) 한
			 * 줄 씩 입력 받는다. : nextLine() -> split() 또는 StringTokenizer // 한명의 성적을 입력 받기 : 오류가
			 * 있으면 오류를 알리고 계속 입력을 받는다. 2. 입력받은 q값이 또는 Q이면 메서드를 종료하고, 그렇지 않으면 입력받은 값으로
			 * Student인스턴스를 생성하고 record에 추가한다. 3. 입력받은 데이터에서 예외가 발생하면, "입력오류입니다."를 보여주고 다시
			 * 입력받는다. 4. q 또는 Q가 입력될 때까지 2~3의 작업을 반복한다.
			 */
			...
			if (lines.contains("q") || lines.contains("Q")) {
				...
				// data[0] : 이름, data[1] : 반, data[2] : 번호, data[3] : 국어, data[4] : 영어
				// data[5] : 수학
				try {
					String name = data[0];
					...
					record.add(student);
					System.out.println("잘입력되었습니다. 입력을 마칠려면 q를 입력하세요.");
